...
class Tvseries extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('imdbRating', 'numerical'),
			array('name, image, Date', 'length', 'max'=>255),
			array('description', 'length', 'max'=>10000),
			array('Country', 'length', 'max'=>100),
			array('imdbVotes', 'length', 'max'=>30),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, description, Country, image, Date, imdbRating, imdbVotes', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('imdbRating',$this->imdbRating);
		$criteria->compare('imdbVotes',$this->imdbVotes,true);
